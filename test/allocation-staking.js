...
describe("AllocationStaking", function () {
    ...
    const END_TIMESTAMP_DELTA = Math.floor(Number(TOKENS_TO_ADD) / Number(REWARDS_PER_SECOND) + 1);
    ...
    async function baseSetup(params) {
        ...
        await AllocationStaking.fund(TOKENS_TO_ADD);//给farm注资

        await AllocationStaking.add(ALLOC_POINT, BreLP1.address, false);//添加LP
    }
    ...
    beforeEach(async function () {
        ...
        C2NToken = await BreTokenFactory.deploy("C2N", "C2N", ethers.parseUnits("1000000000"), 18);

        BreLP1 = await BreTokenFactory.deploy("BreLP1", "BRELP1", ethers.parseUnits("100000000"), 18);
        BreLP2 = await BreTokenFactory.deploy("BreLP2", "BRELP2", ethers.parseUnits("100000000"), 18);
        ...
        await AllocationStaking.initialize(deployer, C2NToken.address, REWARDS_PER_SECOND, startTimestamp, SalesFactory.address);
